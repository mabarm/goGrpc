...
const (
	...
	GreetService_SayHelloServerStreamin_FullMethodName         = "/greet_service.GreetService/SayHelloServerStreamin"
...
type GreetServiceClient interface {
	...
	SayHelloServerStreamin(ctx context.Context, in *NamesList, opts ...grpc.CallOption) (grpc.ServerStreamingClient[HelloResponse], error)
...
func (c *greetServiceClient) SayHelloServerStreamin(ctx context.Context, in *NamesList, opts ...grpc.CallOption) (grpc.ServerStreamingClient[HelloResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &GreetService_ServiceDesc.Streams[0], GreetService_SayHelloServerStreamin_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
type GreetService_SayHelloServerStreaminClient = grpc.ServerStreamingClient[HelloResponse]
...
type GreetServiceServer interface {
	...
	SayHelloServerStreamin(*NamesList, grpc.ServerStreamingServer[HelloResponse]) error
...
func (UnimplementedGreetServiceServer) SayHelloServerStreamin(*NamesList, grpc.ServerStreamingServer[HelloResponse]) error {
	return status.Errorf(codes.Unimplemented, "method SayHelloServerStreamin not implemented")
}
...
func _GreetService_SayHelloServerStreamin_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(NamesList)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(GreetServiceServer).SayHelloServerStreamin(m, &grpc.GenericServerStream[NamesList, HelloResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type GreetService_SayHelloServerStreaminServer = grpc.ServerStreamingServer[HelloResponse]
...
var GreetService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "SayHelloServerStreamin",
			Handler:       _GreetService_SayHelloServerStreamin_Handler,
			ServerStreams: true,
		},
		{
